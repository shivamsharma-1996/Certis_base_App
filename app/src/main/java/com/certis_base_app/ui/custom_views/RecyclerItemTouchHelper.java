...
import com.certis_base_app.ui.createTask.SubtaskTypeAdapter;
...
public class RecyclerItemTouchHelper extends SimpleCallback {
    ...
    public void onSelectedChanged(ViewHolder viewHolder, int i) {
        if (viewHolder != null) {
            ...
            Callback.getDefaultUIUtil().onSelected(((SubtaskTypeAdapter.SubtaskListItemHolder) viewHolder).viewForeground);
        }
    }
    ...
    public void onChildDrawOver(Canvas canvas, RecyclerView recyclerView, ViewHolder viewHolder, float f, float f2, int i, boolean z) {
        Callback.getDefaultUIUtil().onDrawOver(canvas, recyclerView, ((SubtaskTypeAdapter.SubtaskListItemHolder) viewHolder).viewForeground, f, f2, i, z);
    }

    public void clearView(RecyclerView recyclerView, ViewHolder viewHolder) {
        Callback.getDefaultUIUtil().clearView(((SubtaskTypeAdapter.SubtaskListItemHolder) viewHolder).viewForeground);
    }

    public void onChildDraw(Canvas canvas, RecyclerView recyclerView, ViewHolder viewHolder, float f, float f2, int i, boolean z) {
        SubtaskTypeAdapter.SubtaskListItemHolder subtaskListItemHolder = (SubtaskTypeAdapter.SubtaskListItemHolder) viewHolder;
